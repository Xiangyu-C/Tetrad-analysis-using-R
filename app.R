...
server<-function(input, output, session) {
	...
	table_1<-reactiveValues(table_1_pre=0, table_1_cur=1)
	table_2<-reactiveValues(table_2_pre=0, table_2_cur=1)
	table_3<-reactiveValues(table_3_pre=0, table_3_cur=1)
	table_4<-reactiveValues(table_4_pre=0, table_4_cur=1)
	table_5<-reactiveValues(table_5_pre=0, table_5_cur=1)
	table_6<-reactiveValues(table_6_pre=0, table_6_cur=1)
	table_7<-reactiveValues(table_7_pre=0, table_7_cur=1)
	table_8<-reactiveValues(table_8_pre=0, table_8_cur=1)
	table_9<-reactiveValues(table_9_pre=0, table_9_cur=1)
	npd_r_table<-reactiveValues(ratio_table=0)

	observe({
		if(!is.null(input$hot1)) {
			table_1$table_1_cur<-as.data.frame(hot_to_r(input$hot1))
			table_6$table_6_cur<-as.data.frame(hot_to_r(input$hot1))
			table_7$table_7_cur<-as.data.frame(hot_to_r(input$hot1))
		}
		})
	...
	observe({
		if(!is.null(input$hot2)) {
			...
			table_8$table_8_cur<-as.data.frame(hot_to_r(input$hot2))
			table_9$table_9_cur<-as.data.frame(hot_to_r(input$hot2))
		}
		})
	...
	observeEvent(input$analyze2, {
		...
		else if (setequal(selection$last_slt3, selection$curr_slt3) & identical(table_6$table_6_pre, table_6$table_6_cur)
				 & last_btn$counter!=4) {
			updateNavbarPage(session, 'allPanel', selected='GCperc')
		}
		else {
			selection$last_slt3<-input$slt1
			table_6$table_6_pre<-as.data.frame(hot_to_r(input$hot1))
			updateNavbarPage(session, 'allPanel', selected='GCperc')
			new_analysis_temp <- subset(new_analysis_temp, select=input$slt1)
	...
	observeEvent(input$analyze4, {
		...
		else if (setequal(selection$last_slt4, selection$curr_slt4) & identical(table_8$table_8_pre, table_8$table_8_cur)
						  & last_btn$counter!=2) {
			updateNavbarPage(session, 'allPanel', selected='GCperc')
		}
		else {
			selection$last_slt4<-input$slt2
			table_8$table_8_pre<-as.data.frame(hot_to_r(input$hot2))
			updateNavbarPage(session, 'allPanel', selected='GCperc')
			old_analysis_temp <- subset(old_analysis_temp, select=input$slt2)
	...
	observeEvent(input$analyze5, {
		...
		else if (setequal(selection$last_slt6, selection$curr_slt6) & identical(table_9$table_9_pre, table_9$table_9_cur)
					      & last_btn$counter!=6) {
			updateNavbarPage(session, 'allPanel', selected='interference1')
		}
		else {
			selection$last_slt6<-input$slt2
			table_9$table_9_pre<-as.data.frame(hot_to_r(input$hot2))
			updateNavbarPage(session, 'allPanel', selected='interference1')
	...
	observeEvent(input$analyze6, {
		...
		else if (setequal(selection$last_slt5, selection$curr_slt5) & identical(table_7$table_7_pre, table_7$table_7_cur)
						  & last_btn$counter!=5) {
			updateNavbarPage(session, 'allPanel', selected='interference1')
		}
		else {
			selection$slt5<-input$slt1
			table_7$table_7_pre<-as.data.frame(hot_to_r(input$hot1))
			updateNavbarPage(session, 'allPanel', selected='interference1')
